...
for i in range(qtd_modulos):
    with st.expander(f"Módulo #{i+1}"):
        ...
        for cor in cores_escolhidas:
            limite = 9 if tipo_modulo == "Nano" and tipo_led == "3W" and len(cores_escolhidas) == 1 else 3
...
def gerar_pdf(amplificador, valor_amplificador, qtd_driver, valor_driver,
    ...
    if qtd_driver: pdf.cell(200, 10, txt=f"Driver(s): {qtd_driver} - R$ {valor_driver:.2f}", ln=True)
    pdf.cell(200, 10, txt=f"Controlador: {controlador_tipo} - R$ {valor_controlador:.2f}", ln=True)
    pdf.ln(5)
    pdf.set_font("Arial", 'B', 12); pdf.cell(200, 10, txt="Módulos Auxiliares:", ln=True)
    pdf.set_font("Arial", size=12)
    for idx, valor in enumerate(valores_modulos): pdf.cell(200, 10, txt=f"Módulo #{idx+1}: R$ {valor:.2f}", ln=True)
    pdf.cell(200, 10, txt=f"Total Módulos: R$ {valor_total_modulos:.2f}", ln=True)
    pdf.ln(5); pdf.set_font("Arial", 'B', 12)
    pdf.cell(200, 10, txt=f"TOTAL: R$ {total:.2f}", ln=True)
    if img_bytes:
        with open("grafico_temp.png", "wb") as f: f.write(img_bytes.getbuffer())
        pdf.image("grafico_temp.png", x=50, y=None, w=100)
    return pdf.output(dest='S').encode('latin-1')

# --- BOTÃO FLUTUANTE ---
if total > 0:
    st.markdown('<div class="pdf-float-button">', unsafe_allow_html=True)
