...
def calcular_limite_leds(tipo_modulo, tipo_led, cores_escolhidas, cor_atual):
    ...
    if tipo_modulo == "Micro":
        ...
        if tipo_led == "3W":
            limite = 18 if num_cores == 1 else 6
        elif tipo_led == "OPT":
            limite = 12 if num_cores == 1 else 6
        elif tipo_led == "Q-MAX":
            limite = 4
    
    elif tipo_modulo == "Nano" and tipo_led == "3W":
        limite = 9 if num_cores == 1 else 3
...
for i in range(qtd_modelos_modulos):
    with st.expander(f"Modelo de Módulo Auxiliar #{i+1}"):
        ...
        if len(cores_escolhidas) > 0:
            config_led = ["Single", "Dual", "Tri"][len(cores_escolhidas)-1]
        ...
        for cor, qtd in qtd_leds_por_cor.items():
            valor_modulo_unidade += qtd * precos_cor_led[tipo_led][cor]
...
if sinalizador_tipo != "Nenhum":
    ...
    for j in range(qtd_modelos_sinalizador):
        with st.expander(f"Modelo de Módulo Sinalizador #{j+1}"):
            ...
            config_led_s = "Single"
            if len(cores_escolhidas_s) > 0:
                config_led_s = ["Single", "Dual", "Tri"][len(cores_escolhidas_s)-1]
